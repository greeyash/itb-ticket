...
export default async function TicketsPage() {
  ...
  const active = registrations?.filter((r: any) => r.status !== "attended") || [];
  const past = registrations?.filter((r: any) => r.status === "attended") || [];
          ...
          {active.length > 0 && (
            <section>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {active.map((reg) => (
                  <TicketCard key={reg.id} registration={reg as Registration} />
                ))}
              </div>
            </section>
          )}
